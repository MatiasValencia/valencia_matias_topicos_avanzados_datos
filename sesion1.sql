...
-- Ejercicio 1 Sesion 11
BEGIN
    DBMS_OUTPUT.PUT_LINE('Ejercicio 1 Sesion 11');
END;
/
...
BEGIN
    ...
    WHERE ClienteID = p_cliente_id;
    -- Calculo de la edad
    v_edad := FLOOR(MONTHS_BETWEEN(SYSDATE, v_fecha_nacimiento) / 12); -- Formula para calcular edad exacta del cliente
    RETURN v_edad;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20002, 'Cliente con ID ' || p_cliente_id || ' no encontrado.');
...
-- Ejercicio 2 Sesion 11
BEGIN
    DBMS_OUTPUT.PUT_LINE('Ejercicio 1 Sesion 11');
END;
/
CREATE OR REPLACE FUNCTION promedio_total_productos RETURN NUMBER AS
    v_promedio NUMBER;
BEGIN
...
WHERE Precio < promedio_total_productos();

-- Ejercicio 1 Sesion 12
BEGIN
    DBMS_OUTPUT.PUT_LINE('Ejercicio 1 Sesion 12');
END;
/
-- Funcion
...
BEGIN
    ...
    IF Total > 1000 THEN
        v_total := v_total - (v_total * 10 / 100);
    END IF;
    RETURN v_total;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20004, 'Pedido con ID ' || p_cliente_id || ' no encontrado.');
END;
/
...
BEGIN
    v_total_nuevo := calcular_total_con_descuento(p_pedido_id);
    UPDATE Pedidos
    SET Total = v_total_nuevo
    WHERE PedidoID = p_pedido_id;
    DBMS_OUTPUT.PUT_LINE('Total del pedido con ID ' || p_pedido_id || ' actualizado a $' || v_total_nuevo);
    COMMIT;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20004, 'Pedido con ID ' || p_cliente_id || ' no encontrado.');
...
-- Ejercicio 2 Sesion 12
BEGIN
    DBMS_OUTPUT.PUT_LINE('Ejercicio 2 Sesion 12');
END;
/
...
BEGIN
    IF :NEW.Cantidad <= 0 THEN
        DBMS_OUTPUT.PUT_LINE('La cantidad debe ser mayor a 0.');
    END IF;
END;
/
...
BEGIN
    FOR detalle IN detalle_cursor LOOP  -- Bucle para cada detalle dentro del cursor
        -- Verificamos cantidad disponible
        SELECT Cantidad INTO v_cantidad_actual FROM Inventario
        WHERE ProductoID = detalle.ProductoID;

        SAVEPOINT antes_de_reducir;  -- Creamos nuestro primer savepoint antes de reducir cantidad

        IF v_cantidad_actual < detalle.Cantidad THEN
            RAISE_APPLICATION_ERROR(-20001, 'No hay suficiente inventario.');
        END IF;
        ...
        DBMS_OUTPUT.PUT_LINE('Inventario para el producto ' || detalle.ProductoID || ' ha sido actualizado.');
    END LOOP;
    COMMIT;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Error: Producto no encontrado en el inventario.');
        ROLLBACK;
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
        ROLLBACK TO antes_de_reducir;
        COMMIT;
END;
/

-- Ejercicio 2 Sesion 13
BEGIN
    DBMS_OUTPUT.PUT_LINE('Ejercicio 2 Sesion 13');
END;
/
-- Creacion dimension Dim_Ciudad
...
-- Creacion dimension Dim_Cliente
...
INSERT INTO Dim_Tiempo (FechaID, Fecha, Anio, Mes, Dia)
...
-- Creacion tabla de hechos Fact_Pedidos
CREATE TABLE Fact_Hechos (
    PedidoID NUMBER PRIMARY KEY,
    ClienteID NUMBER,
    CiudadID NUMBER,
    FechaID NUMBER,
...
SELECT p.PedidoID, p.ClienteID, dc.CiudadID, dt.FechaID, p.Total FROM Pedidos p_cantidad
